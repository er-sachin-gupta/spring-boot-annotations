...
//    @RequestMapping(value = "/books/delete/{id}", method = RequestMethod.DELETE)
    ...
    public ResponseEntity<Book> pathVariableDemo(@PathVariable(value = "id") int id,
                                                 @PathVariable("title") String bookTitle,
                                                 @PathVariable(name = "description") String bookDesc) {
        System.out.println(id);
        Book book = new Book();
        book.setId(id);
    ...
    // http://localhost:8080/api/books/query?id=1&title=RamCharitManas
    @GetMapping("/books/query")
    public ResponseEntity<Book> requestParamsDemo(@RequestParam(name = "id") int id,
